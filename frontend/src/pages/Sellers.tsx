...
/** Scroll reveal for section H2; IO only when motion is OK; CSS motion-reduce: also forces visible state. */
    ...
    >
      <div className={SECTION_CLASS}>
        <div
          ref={titleBlockRef}
          className={
            "mb-8 sm:mb-10 transition-[opacity,transform] duration-700 ease-out motion-reduce:transition-none " +
            (titleVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2") +
            " motion-reduce:opacity-100 motion-reduce:translate-y-0"
          }
        >
          <h2
            className="text-2xl sm:text-3xl font-semibold tracking-tight leading-snug max-w-2xl"
...
/** Narrative subsection (H3 + paragraphs) — typographic only, no rail/box. */
...
/** Same typography as NarrativeBlock; children = short paragraphs + optional lists. */
...
function SellersHeader({ onSignOut }: { onSignOut: () => void }) {
  ...
  return (
    <header
      className="border-b fixed top-0 left-0 right-0 z-20 backdrop-blur-sm bg-white/90"
      style={{ borderColor: tokens.border, height: SELLERS_HEADER_HEIGHT }}
    >
      <div className="max-w-4xl mx-auto px-3 sm:px-6 h-full flex items-center justify-between gap-2 sm:gap-4">
        ...
        <nav
          className="flex items-center justify-center gap-0 sm:gap-1 overflow-x-auto overflow-y-hidden flex-nowrap min-w-0 flex-1 no-scrollbar py-0.5"
          aria-label="Sidsektioner"
        >
          {ANCHOR_LINKS.map(({ href, labelKey }) => (
            <a
              key={href}
              href={href}
              className="px-1.5 sm:px-2 py-1.5 text-[11px] sm:text-xs font-medium whitespace-nowrap rounded-md transition-colors hover:bg-black/[0.04]"
        ...
        <button
          type="button"
          onClick={onSignOut}
          className="flex items-center gap-1.5 sm:gap-2 min-h-[44px] px-2 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors shrink-0 hover:bg-black/[0.04]"
          style={{ color: tokens.text }}
          title={t.lockPage}
        >
          <Lock className="h-4 w-4 shrink-0" style={{ color: tokens.textMuted }} aria-hidden />
          <span className="sm:hidden">{t.lockPageShort}</span>
          <span className="hidden sm:inline">{t.lockPage}</span>
        </button>
      </div>
    </header>
  );
    ...
    <div className="min-h-screen overflow-x-hidden" style={{ backgroundColor: tokens.bg }} data-design-profile="nivo">
      ...
      <div style={{ paddingTop: SELLERS_HEADER_HEIGHT }}>
        ...
        <section className="w-full" style={{ background: tokens.gradients.soft }}>
          <div className={SECTION_CLASS}>
            <figure className="max-w-3xl mx-auto py-6 sm:py-10">
              <blockquote
        ...
        <section className="w-full" style={{ backgroundColor: tokens.bg }}>
          <div className={SECTION_CLASS}>
            <div className="max-w-2xl mx-auto">
              ...
              <BulletTextRows
                lines={[t.introP1, t.introP2, t.introP3, t.introP4, t.introP5, t.introP6]}
                size="15"
              />
            </div>
          </div>
        ...
        <SectionShell title={t.kontaktSectionTitle} id="kontakt">
          ...
          <p className="text-[15px] sm:text-base font-semibold tracking-tight mb-4 max-w-2xl" style={{ color: tokens.text }}>
            {t.kontaktDuKan}
          </p>
          <div
          ...
          <p className="text-[15px] sm:text-base leading-relaxed max-w-2xl" style={{ color: tokens.textMuted }}>
            {t.kontaktFooter}
          </p>
        </SectionShell>
      </div>
