 ...
 * - If not bootstrapped (empty DB): lets them claim the first admin account via POST /api/bootstrap.
 * - If bootstrapped (other users exist): shows a "pending approval" message.
 */
...
import { Loader2, Shield, Clock, Wifi, ChevronDown, ChevronUp } from 'lucide-react'

export default function ClaimFirstAdmin() {
  const { user, isBootstrapped, signOut } = useAuth()
  ...
  const handleClaim = async () => {
    ...
    try {
      await postBootstrap()
      // Reload so /api/enroll is refetched and userRole is set
