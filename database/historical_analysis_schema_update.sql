-- Historical Analysis Tab Redesign - Database Schema Updates
-- Add template/focus tracking to ai_analysis_runs table
...
-- Create index for efficient querying by template and date
...
COMMENT ON COLUMN ai_ops.ai_analysis_runs.analysis_template_id IS 'ID of the analysis template used (if any)';
COMMENT ON COLUMN ai_ops.ai_analysis_runs.analysis_template_name IS 'Name of the analysis template used (if any)';
COMMENT ON COLUMN ai_ops.ai_analysis_runs.custom_instructions IS 'Custom instructions provided by user (if no template used)';
COMMENT ON COLUMN ai_ops.ai_analysis_runs.company_count IS 'Number of companies analyzed in this run';
